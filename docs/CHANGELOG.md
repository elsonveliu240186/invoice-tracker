...
### Added
- **True E2E smoke + regression suite — full-stack boot, industry-standard coverage** (FEAT-20260518-01) — _no breaking changes, no new production API endpoints_:
  Adds a dedicated real-stack E2E layer on top of the existing mocked Playwright suite. All existing mocked specs (`page.route()`) are completely untouched. Two tiers:
  **Smoke** (every PR + push, ≤ 3 min, Chrome only) and **Regression** (nightly + push to `main`, ≤ 12 min, Chrome + Firefox + mobile viewport 390×844).
  Infrastructure: `docker-compose.e2e.yml` boots four services with health-checks — Postgres 16 on tmpfs (no volume), MailHog (SMTP `:1026`, REST `:8026`), Spring Boot backend (`SPRING_PROFILES_ACTIVE=e2e`, port 8082), and frontend nginx (port 8081); `COMPOSE_PROJECT_NAME=invoice-tracker-e2e` prevents port collision with the local dev stack. `.env.e2e` commits fixed throwaway test credentials (`POSTGRES_PASSWORD=e2e`, `API_PASSWORD=Secret1!`).
  Backend `e2e` profile: `application.yml` `on-profile: e2e` document sets `flyway.clean-disabled=false`, datasource → `postgres:5432/invoicetracker_e2e`, mail host `mailhog:1025`. `FlywayCleanMigrateInitializer` (`@Configuration @Profile("e2e")`) implements `FlywayMigrationStrategy` — calls `flyway.clean()` then `flyway.migrate()` on every container start for a fresh schema. `E2eResetController` (`@RestController @Profile("e2e")`) exposes `POST /api/v1/test-support/reset` (HTTP Basic auth required) — truncates business tables in FK order (`invoice_generated_artifacts`, `invoice_lines`, `invoices`, `expenses`, `clients`, `app_users`, `company_profile`) and returns 204. The endpoint returns 404 in all non-`e2e` profiles (enforced by `E2eResetControllerProfileGuardIT`). Four new backend test classes: `FlywayCleanMigrateInitializerTest` (Mockito: verifies clean+migrate order), `E2eResetControllerTest` (MockMvc: 204 with auth, 401 anon), `E2eResetControllerProfileGuardIT` (no e2e profile → 404), `E2eResetControllerIT` (seed + reset → counts = 0).
  Playwright infrastructure: `playwright.config.ts` gains two named projects — `smoke` (Chromium, `tests/e2e/smoke`) and `regression` (Chromium + Firefox, `tests/e2e/regression`); both excluded from the default `pnpm e2e` command. `global-setup.ts` registers the admin user and purges MailHog inbox. Fixture module `tests/e2e/fixtures/test.ts` extends `test` with a `beforeEach` that calls `resetBackend()` + `purgeMailhog()`. `TestDataFactory` (`fixtures/factory.ts`) provides typed builders: `createClient`, `createInvoice`, `createExpense`, `saveCompanyProfile` — each POSTs to the real API and asserts 2xx. Raw HTTP helpers live in `fixtures/api.ts`.
  9 Page Object Model classes under `tests/e2e/pages/`: `LoginPage`, `AppShellPage`, `ClientsPage`, `InvoicesPage`, `InvoiceDetailPage`, `ExpensesPage`, `DashboardPage`, `SettingsCompanyPage`, `SettingsTemplatePage` — all methods are semantically named (no raw locators in spec files).
  7 smoke specs: `auth.spec.ts` (register → login → `/` → logout → `/login`), `clients.spec.ts` (create → visible in list), `invoices.spec.ts` (create → DRAFT badge), `send-email.spec.ts` (send invoice → MailHog 1 message), `expenses.spec.ts` (create → visible in list), `dashboard.spec.ts` (KPI cards with seeded data), `settings.spec.ts` (upload `.docx` → metadata visible).
  12 regression specs: `auth.spec.ts` (wrong password; session refresh; logout; protected-route redirect), `clients.spec.ts` (full CRUD; 409 duplicate; search; pagination 25 clients), `invoices-crud.spec.ts` (multi-line; edit; delete; 409 duplicate number; 400 zero lines; totals), `invoices-lifecycle.spec.ts` (DRAFT → SENT → PAID; re-send blocked), `invoices-docx.spec.ts` (DOCX byte scan — feature-guarded on FEAT-20260518-02), `invoices-send.spec.ts` (PDF attachment; null email 422; subject matches number), `expenses.spec.ts` (CRUD; negative 400; filters; pagination), `dashboard.spec.ts` (chart bars and legend; date filter), `settings-company.spec.ts` (8-field save/reload; empty name; IBAN too long), `settings-template.spec.ts` (upload valid; `.exe` 415; >5 MB; download), `navigation.spec.ts` (sidebar links; active highlight; mobile drawer; 404), `accessibility.spec.ts` (axe-core on 6 key pages; zero critical/serious violations).
  CI: two new GitHub Actions jobs added to `.github/workflows/ci.yml` — `e2e-smoke` (`needs: [backend, frontend]`, runs on every PR + push, uploads Playwright HTML report + backend logs on failure) and `e2e-regression` (schedule `0 2 * * *` + push to `main`, Chrome + Firefox, uploads full trace on failure). `CHECKS.yml` updated with `e2e_smoke: true` and `e2e_regression: true`. MailHog email polling uses 5 s / 250 ms intervals; `retries: 1` on CI for flake tolerance.
  Security: `E2eResetController` is inaccessible in production (404 via `@Profile("e2e")` + 401 via HTTP Basic enforced by global `SecurityConfig`); `.env.e2e` contains only throwaway test credentials with no access to any real data store; `flyway.clean-disabled=false` is scoped to the `e2e` YAML document only; `@axe-core/playwright` is a devDependency never bundled in the production build; MailHog is bound to the compose network only. QA: 333 backend unit tests + 80 IT tests pass (Maven `verify`); 1004 Vitest tests pass; Playwright smoke/regression specs verified to parse and enumerate correctly.
