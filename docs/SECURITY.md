...
| ID | Feature | Finding | Severity | Status |
|----|---------|---------|----------|--------|
| OF-1 | FEAT-20260512-03 | `nginx.conf` missing security headers: `X-Frame-Options`, `X-Content-Type-Options`, `Content-Security-Policy`, `Referrer-Policy`. Pre-existing gap since FEAT-20260512-01. | Medium | Open — resolve in `FEAT-security-hardening` before production. |
| OF-2 | FEAT-20260512-03 | `vite >=6.4.2` upgrade needed for CVE-2026-39365/GHSA-4w7w-66w2-5vf9 (dev-server path traversal). `esbuild >=0.25.0` for GHSA-67mh-4wv8-2f99. Dev-only; no production impact. | Low | Open — upgrade before go-live. |
| OF-3 | FEAT-20260512-03 | `localStorage` Basic token (R-1, carried from FEAT-20260512-02). | Medium | Open — migrate to httpOnly cookie in `FEAT-auth-cookies`. |
...
|------|---------|---------|----------|------------|-----------|
<<<<<<< HEAD
| 2026-05-14 | FEAT-20260513-02 Invoice PDF generation and email delivery | Security audit passed (FEAT-03 same-day baseline). 0 required fixes. 7 non-blocking recommendations: nginx security headers (X-Frame-Options, X-Content-Type-Options, CSP, Referrer-Policy); extend external-rel check to UNC/file:// URIs; add `@Pattern(regexp = "[^\r\\n]+")` on `CreateInvoiceRequest.number`; rate limiting on send-email (5 req/invoice/min); upgrade esbuild ≥ 0.25.0 and vite ≥ 6.4.2; add `frontend/.env.local` to `.gitignore`; set `NVD_API_KEY` as GitHub Actions secret. | Low | 0 required fixes accepted. 7 recommendations tracked as follow-ups. All 10 OWASP categories mitigated — CRLF guard on invoice.number and client.email; email logged as SHA-256 trunc-8; SMTP creds from env vars only; PDF marked Cache-Control: private, no-store; no dangerouslySetInnerHTML. | [SECURITY.md](.features/FEAT-20260513-02/SECURITY.md) |
=======
| 2026-05-13 | FEAT-20260512-03 Dashboard and core UI modernization | No required fixes. 0 High/Critical CVEs (2 pre-existing Moderate carried from FEAT-20260512-01: esbuild GHSA-67mh-4wv8-2f99 CVSS 5.3 and vite GHSA-4w7w-66w2-5vf9 / CVE-2026-39365 — dev-server only). All OWASP Top 10 categories reviewed by auditor. Four non-blocking recommendations: add nginx security headers; upgrade vite/esbuild before production; migrate `localStorage` Basic token to httpOnly cookie; swap hard-coded English strings in `ClientStatusBadge` and `ConfirmDeleteDialog` to i18n keys. | Low | All non-blocking. Resolve OF-1, OF-2 in `FEAT-security-hardening` before production. | [SECURITY.md](.features/FEAT-20260512-03/SECURITY.md) |
