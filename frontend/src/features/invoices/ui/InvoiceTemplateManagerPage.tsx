...
export function InvoiceTemplateManagerPage() {
  const { t } = useTranslation();
  const { data: metadata, loading, refetch } = useTemplateMetadata();

  return (
    <div data-testid="invoice-template-manager-page">
      {/* Back link */}
      <div className="mb-4">
        <Link
          to="/invoices"
        ...
        >
          ...
          {t('invoices.template.back')}
        </Link>
      </div>

      <h1 className="mb-6 text-2xl font-bold text-[var(--color-foreground)]">
        {t('invoices.template.title')}
      </h1>

      <p className="mb-6 text-sm text-[var(--color-muted-foreground)]">
        {t('invoices.template.intro')}
      </p>

      <Card className="mb-6">
        <CardHeader>
          <CardTitle>{t('invoices.template.title')}</CardTitle>
        </CardHeader>
